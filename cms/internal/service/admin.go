...
func (s *Service) AdminTokenDel(ctx context.Context, token string) error {
...
func (s *Service) AdminUserFindByName(ctx context.Context, name string) (*entity.AdminUser, error) {
	...
	err := s.mysql.WithContext(ctx).Where("username = ?", name).
		Preload("AdminRole").Take(&result).Error
...
func (s *Service) AdminUserUpdate(ctx context.Context, data *entity.AdminUser) error {
	opt := s.mysql.WithContext(ctx).Updates(data) // 根据ID更新非零值字段
	if opt.Error != nil {
		return opt.Error
	}
...
func (s *Service) AdminUserLogout(ctx context.Context, id int) error {
	...
	if token != "" {
		return s.redis.Del(ctx, ssoKey, cache.AdminTokenKey(token)).Err()
	}
	return nil
}
...
func (s *Service) AdminRoleUpdate(ctx context.Context, data *entity.AdminRole) error {
	return s.mysql.WithContext(ctx).Updates(data).Error // 根据ID更新非零值字段
}
