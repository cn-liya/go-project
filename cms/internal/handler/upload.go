...
func (h *Handler) UploadImage(c *gin.Context) {
	...
	if f.Size > 500<<10 {
		c.JSON(bizresp.EntityTooLarge.WithMsg("单张图片限制500KB以内"))
		return
	}
	file, _ := f.Open()
	defer file.Close()
	...
	if ext == "" {
		c.JSON(bizresp.UnsupportedMediaType.WithMsg("无效的图片类型，仅支持jpg/png/gif格式"))
		return
	}
...
func (h *Handler) UploadAudio(c *gin.Context) {
	...
	if f.Size > 2<<20 {
		c.JSON(bizresp.EntityTooLarge.WithMsg("单个文件限制2M以内"))
		return
	}
	file, _ := f.Open()
	defer file.Close()
	...
	if ext == "" {
		c.JSON(bizresp.UnsupportedMediaType.WithMsg("无效的音频类型，仅支持mp3/wav/oga格式"))
		return
	}
...
func (h *Handler) UploadVideo(c *gin.Context) {
	...
	if f.Size > 5<<20 {
		c.JSON(bizresp.EntityTooLarge.WithMsg("单个文件限制5M以内"))
		return
	}
	file, _ := f.Open()
	defer file.Close()
	...
	if ext == "" {
		c.JSON(bizresp.UnsupportedMediaType.WithMsg("无效的视频类型，仅支持mp4/webm/ogv格式"))
		return
	}
...
func (h *Handler) UploadPDF(c *gin.Context) {
	...
	if f.Size > 1<<20 {
		c.JSON(bizresp.EntityTooLarge.WithMsg("单个文件限制1M以内"))
		return
	}
	file, _ := f.Open()
	defer file.Close()
	...
	if http.DetectContentType(b) != "application/pdf" {
		c.JSON(bizresp.UnsupportedMediaType.WithMsg("无效的文件类型，仅支持pdf格式"))
		return
	}
