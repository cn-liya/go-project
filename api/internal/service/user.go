...
func (s *Service) UserSave(ctx context.Context, data *entity.User) (int, error) {
	...
	if err = s.redis.Set(ctx, cache.UserInfoKey(data.ID), b, time.Hour).Err(); err != nil {
		logger.FromContext(ctx).Error("redis.Set", nil, err)
	}
	return data.ID, nil
}
...
func (s *Service) UserUpdate(ctx context.Context, data *entity.User) error {
	opt := s.mysql.WithContext(ctx).Updates(data) // 根据ID更新指定非零值字段
	if opt.Error != nil {
		return opt.Error
	}
...
func (s *Service) AvatarToCdnAsync(data *queue.AvatarToCdn) error {
	b, _ := json.Marshal(data)
	return s.producer.Publish(queue.AvatarToCdnQN, b)
}
